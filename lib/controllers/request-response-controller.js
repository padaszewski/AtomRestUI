...
function createViewRes(resData, status, startTime) {
  return {
    time: getTimeDiff(startTime, (new Date).getTime()),
    status: status,
...
export default class RequestResponseController {
        ...
        switch (req.type) {
            case "GET":
                axios.get(req.url)
                    .then((response) => {
                        // handle success
                        const res = createViewRes(response.data, response.status , startTime);
    ...
    handleError(error, request) {
        if (error.response) {
            ...
            const res = createViewRes(message, error.response.status , startTime);
            this.showResponse(res);
            ...
            const res = createViewRes(message, "", startTime);
            this.showResponse(res);
        }
    }
